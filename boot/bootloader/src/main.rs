...
extern "C" fn main(sector: u8) -> ! {
	...
	// TODO: ELF loading - Will become frieren::parse_from_sector(sector)
	...
	// TODO: BIOS calls tracked the cursor location before, so when we stop using BIOS we lose the cursor
	// location and start printing from the top. Need to find a way to get the cursor location from BIOS.
...
/// but the OSDev wiki documents a method to jump straight from 16-bit mode to 64-bit mode:
